...
if (isset($_POST['Envoyer'])) {
    ...
    if ($compare) {  // Cas où les données de connexion sont trouvées dans la table utilisateurs
        ...
        if ($role_id == 3) {
            echo "<script type=\"text/javascript\"> window.location='../Profils/PHP/visiteur_medical.php' </script>";
        } elseif ($role_id == 2) {
            echo "<script type=\"text/javascript\"> window.location='../Profils/PHP/dashbord.php' </script>";
        }elseif ($role_id == 1) {
            echo "<script type=\"text/javascript\"> window.location='../Profils/PHP/admin.php' </script>";
        }else {
            echo "<script type=\"text/javascript\">window.alert ('Rôle non autorisé'); window.location='./Connection/connexion_gsb.html' </script>";
        }
    } else {
